...
export async function createPptOutline(input, { signal } = {}) {
  const url = endpoint('/api/skills/ppt/outline');
  if (!url) throw new Error('离线单文件模式无法调用 PPT 服务，请启动服务端后重试。');
...
export async function renderPpt(spec, { signal } = {}) {
  const url = endpoint('/api/skills/ppt/render');
  if (!url) throw new Error('离线单文件模式无法渲染 PPTX，请启动服务端后重试。');
  const response = await fetchPptService(url, {
    method: 'POST',
