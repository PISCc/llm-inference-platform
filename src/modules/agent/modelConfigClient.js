...
function endpoint(path) {
  ...
  if (import.meta.env.DEV) return 'http://127.0.0.1:8787' + path;
  return path;
}
...
export async function fetchModelConfigStatus({ signal } = {}) {
  const url = endpoint('/api/agent/config/status');
  if (!url) return { config: { configured: false, source: 'offline' } };
...
export async function saveModelConfig(input, { signal } = {}) {
  ...
  if (!url) throw new Error('离线单文件模式无法保存在线模型配置，请启动服务端后重试。');
...
export async function testModelConfig(input = {}, { signal } = {}) {
  const url = endpoint('/api/agent/config/test');
  if (!url) throw new Error('离线单文件模式无法测试在线模型，请启动服务端后重试。');
  const response = await fetch(url, {
    method: 'POST',
    headers: modelConfigHeaders({ 'Content-Type': 'application/json' }),
    body: JSON.stringify(input),
