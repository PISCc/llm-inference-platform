...
import { ArrowUpRight, Bot, Calculator, Clock3, Cpu, FlaskConical, Network, Scale, ScanSearch } from 'lucide-react';
...
const MODULES = [
  ...
  { to: '/hardware', title: '硬件容量计算器', desc: '根据模型权重、峰值缓存、智能体和图片存储，反推可运行的硬件搭配。', icon: Calculator, tone: 'blue', tags: ['峰值余量', '存储暂存', '自动配型'] },
    ...
    <div className="warm-home">
      <section className="warm-hero" aria-labelledby="home-title">
        <div className="warm-hero__copy">
          ...
          <div className="warm-capabilities" aria-label="平台能力">
            {['知识全景', '过程模拟', '参数实验', '方案对比', '技术问答', '硬件规划', '链路诊断'].map((item) => <span key={item}>{item}</span>)}
          </div>
          <dl className="warm-facts">
            <div><dt>{MODULE_COUNT}</dt><dd>核心技术模块</dd></div>
            <div><dt>7</dt><dd>互动产品模块</dd></div>
      ...
      <section className="warm-modules" aria-labelledby="modules-title">
        <header className="warm-section-head">
          <h2 id="modules-title">七种方式，进入推理系统。</h2>
          <p>从整体知识结构进入，再观察执行过程、调整参数、比较方案、估算硬件、定位问题，或从一个具体问题开始。</p>
        </header>
            ...
            return (
              <Link key={module.to} to={module.to} className={`warm-module-card warm-module-card--${module.tone}`}>
                <div className="warm-module-card__top">
                  ...
                  <span className="warm-module-card__index">{String(index + 1).padStart(2, '0')} / 07</span>
                </div>
                <h3>{module.title}</h3>
                <p>{module.desc}</p>
