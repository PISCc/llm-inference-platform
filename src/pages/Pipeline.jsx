...
function StageNode({ index, label, active, done, stageKey, menuOpen, onOpenMenu }) {
  return (
    <div className="pipeline-stage-node flex flex-1 flex-col items-center gap-1.5">
      <button onClick={() => onOpenMenu(stageKey)} aria-haspopup="menu" aria-expanded={menuOpen}
...
export default function Pipeline() {
  ...
  const pageContext = useMemo(() => ({
    ...
    selection: {
      ...
      caseDefinition: selectedCase?.answer || null,
    ...
    visibleSummary: selectedCase
      ...
      : '尚未选择演示案例，当前处于输入准备阶段。',
    suggestedQuestions: [
      `当前${activeStage.label}阶段在做什么？`,
      stage === 'branch' ? '有无 KV Cache 的计算差异是什么？' : '当前阶段会影响哪个推理指标？',
      stage === 'finished' && stats
        ? '如何解读当前分支与另一分支的结构计数？'
        : selectedCase ? `结合当前案例解释${selectedCase.label}。` : '如何选择一个案例开始演示？',
    ],
    boundaries: [],
              ...
              <motion.div key="idle" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="pipeline-idle-state">
                <div className="pipeline-idle-guide">
                  ...
                  <div>
                    ...
                    <p>{selectedCase ? '确认问题与 Batch 配置后开始。动画会从“分词”阶段自动播放，也可以随时暂停或单步推进。' : '从预设知识问题开始，平台会把请求依次带入分词、Prefill、KV Cache 决策与 Decode。'}</p>
                  </div>
                </div>
