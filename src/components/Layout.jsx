...
import { Cpu, Network, FlaskConical, Scale, ScanSearch, Bot, Calculator, Home, Menu, X } from 'lucide-react';
...
const PAGE_META = {
  ...
  '/hardware': { title: '硬件容量计算器 · LLM 推理工作台', description: '根据模型权重、缓存、智能体和图片存储，反推可行硬件搭配。' },
...
const NAV = [
  ...
  { to: '/hardware', label: '硬件计算器', icon: Calculator },
    ...
    <div className="workbench-app relative flex min-h-screen flex-col">
      ...
      <header className="workbench-header sticky top-0 z-30">
        <div className="workbench-header__inner">
          ...
          <nav id="primary-navigation" className={cn('workbench-nav', mobileNavOpen && 'is-open')} aria-label="主导航">
            {NAV.map((item) => {
