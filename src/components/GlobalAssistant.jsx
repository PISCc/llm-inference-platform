...
export default function GlobalAssistant() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      ...
      const focusable = [...dialogRef.current.querySelectorAll('button, textarea, a[href], input, select, [tabindex]:not([tabindex="-1"])')]
    ...
    <>
      ...
      <AnimatePresence>
        ...
        <motion.section
              ...
              aria-describedby="assistant-dialog-context"
              initial={{ opacity: 0, y: 14, scale: 0.985 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, y: 10, scale: 0.99 }}
              transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }}
              className="global-assistant-dialog fixed inset-x-0 bottom-0 z-50 flex max-h-[82dvh] flex-col overflow-hidden rounded-t-[1.4rem] border border-space-700 bg-space-900 shadow-[0_-18px_60px_rgba(67,58,46,0.18)] md:inset-auto md:right-5 md:top-[4.75rem] md:h-[min(680px,calc(100vh-6rem))] md:w-[420px] md:max-h-none md:rounded-2xl md:shadow-[0_20px_70px_rgba(67,58,46,0.18)]"
            >
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-2.5">
                    ...
                    <div className="min-w-0">
                      <h2 className="text-sm font-semibold text-space-100">AI 助手</h2>
                       <p id="assistant-dialog-context" className="mt-0.5 truncate text-[10px] text-space-500">{pageContext.pageTitle || '大模型推理互动展示平台'}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1">
                    <button
                      type="button"
                      aria-label="配置模型服务"
                      title={modelStatus.configured ? `${modelStatus.source === 'session' ? '会话模型' : '部署默认'}：${modelStatus.model}` : '尚未配置在线模型'}
