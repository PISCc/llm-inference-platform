        ...
        >
          <header className="flex items-start justify-between gap-4 border-b border-space-700/80 px-5 py-4 md:px-6">
            <div>
              ...
              <p className="mt-1.5 max-w-xl text-xs leading-5 text-space-500">填写你的 API Key、模型和服务地址。密钥只在服务端运行时使用。</p>
            </div>
          ...
          <form onSubmit={submit} className="space-y-5 p-5 md:p-6">
            {typeof window !== 'undefined' && window.location.protocol === 'file:' && (
              <div className="rounded-xl border border-amber-500/25 bg-amber-500/[0.07] px-3.5 py-3 text-xs leading-5 text-amber-500">
                当前以本地文件方式打开：AI 助手会调用本地服务 <span className="font-mono text-amber-400">http://127.0.0.1:8787</span>。请先在项目目录运行
                <span className="mx-1 rounded bg-amber-500/10 px-1.5 py-0.5 font-mono text-amber-400">npm start</span>
                启动服务端，再填写并保存你的 API 配置。
              </div>
            )}
            ...
            <label className="block text-xs text-space-400">
              ...
              <span className="mt-1.5 block text-[11px] text-space-600">OpenAI 兼容服务通常填写到 `/v1`；本地 vLLM 可填写 `http://127.0.0.1:8000/v1`（默认策略会拒绝私网地址，需服务端设置 `ALLOW_PRIVATE_MODEL_ENDPOINTS=true`）。</span>
            </label>
            ...
            <div className="rounded-xl border border-cyan-500/15 bg-cyan-500/[0.05] p-3 text-[11px] leading-5 text-space-500">
              ...
              <p className="mt-1">API Key 不会写入页面上下文、对话历史或浏览器存储。配置以加密令牌在请求间传递；服务端未配置 `MODEL_CONFIG_SECRET` 时仅保存在服务进程内，重启或多实例部署后需重新输入。</p>
            </div>

            {(error || notice) && (
              <div className={`flex items-start gap-2 rounded-xl border p-3 text-xs leading-5 ${error ? 'border-rose-500/20 bg-rose-500/[0.06] text-rose-200' : noticeTone === 'warning' ? 'border-amber-500/25 bg-amber-500/[0.08] text-amber-200' : 'border-emerald-500/20 bg-emerald-500/[0.06] text-emerald-200'}`}>
                {error ? <AlertTriangle size={14} className="mt-0.5 shrink-0" /> : noticeTone === 'warning' ? <AlertTriangle size={14} className="mt-0.5 shrink-0" /> : <CheckCircle2 size={14} className="mt-0.5 shrink-0" />}
