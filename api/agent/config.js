...
export function agentCorsHeaders(request, env = process.env) {
...
export async function handleAgentConfigTestRequest(request, env = process.env) {
  ...
  try {
    ...
    return jsonResponse({ error: { code: error.code || 'MODEL_TEST_FAILED', message: error.status && error.status >= 500 ? '模型连接测试失败，请检查服务地址、Key 和模型名称。' : error.message } }, Number(error.status) || 502, request, env);
  }
}
