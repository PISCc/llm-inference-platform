...
面向大模型推理系统的交互式技术工作台，集成技术全景图、推理流程动画、参数实验、方案对比、硬件容量规划、证据驱动诊断、上下文 AI 问答与 PPT 导出。当前版本包含 7 个互动产品模块，并支持单文件离线浏览。
...
仓库同时保留独立的 `dist/hardware-calculator-mvp.html`，用于不启动 React 工作台时快速验证硬件计算器的输入输出；正式工作台入口为 `dist/index.html`。如需使用项目约定的版本文件名查看，可在构建后将 `dist/index.html` 复制为 `dist/llm-inference-platform-v5.html`。
