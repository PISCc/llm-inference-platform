...
function normalizeBaseUrl(value, { apiKey = '', provider = '' } = {}) {
...
export function normalizeModelConfig(input = {}, { existing = null } = {}) {
  ...
  const baseUrl = normalizeBaseUrl(input.baseUrl === undefined ? existing?.baseUrl : input.baseUrl, { apiKey, provider });
...
export function getConfigToken(request) {
  return clean(request?.headers?.get('x-agent-config-token') || '', 160);
}

export function getStoredModelConfig(token) {
  purgeExpired();
...
export function getModelConfigForRequest(request, env = process.env) {
  const configured = getStoredModelConfig(getConfigToken(request));
  return configured || null;
}

export function upsertModelConfig(input, token = '') {
  purgeExpired();
  const entries = store();
  const current = (token && entries.get(token)?.config) || null;
  const config = normalizeModelConfig(input, { existing: current });
