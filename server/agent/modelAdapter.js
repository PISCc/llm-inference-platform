...
export function createModelAdapter({ env = process.env, config: configOverride = null, fetchImpl = fetch } = {}) {
  ...
  return {
    ...
    async testConnection({ signal } = {}) {
      // 与真实对话走同一条流式路径，避免“非流式可用、流式不可用”或反之的假阴性。
      let received = '';
      for await (const delta of this.streamAnswer({
        messages: [{ role: 'user', content: '请只回复 OK。' }],
        signal,
      })) {
        received += delta;
        if (received.trim()) break;
      }
      if (!received.trim()) {
        throw new ModelAdapterError('模型返回了空回答。', { code: 'MODEL_EMPTY_RESPONSE', status: 502 });
      }
      return { ok: true };
    },

    async *streamAnswer({ messages, signal, overrides } = {}) {
      const response = await requestCompletion({ messages, stream: true, signal, overrides });
